...
Your goal is to verify that the changes currently present in the working directory (unstaged it changes) match the requirements and specifications defined in the attached **Implementation Plan**.
...
Create the file defined in the plan's `Report Structure` section (e.g., `ai-plans/{YYMMDD}__IMPLEMENTATION_REPORT__{name}.md`).
