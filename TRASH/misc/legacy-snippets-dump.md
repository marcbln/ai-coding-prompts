...
## Plan Structure
- The First line of the plan should be the filename of the plan in format `ai-backlog/plans{YYMMDD}__PLAN__{name-of-the-plan}.md`
- Include YAML frontmatter with the following fields:
```yaml
  ---
  title: "Descriptive title of the plan"
...
- Include source code in the plan, mark each code block as [NEW FILE], [MODIFY], or [DELETE] to show the type of change
- The plan should also include an update of the user documentation, if needed.
- The last phase of the plan should be to write a report of the changes that were made to a file in the `ai-backlog/plans{YYMMDD}__IMPLEMENTATION_REPORT__{name-of-the-plan}.md` directory.
- Please follow SOLID principles
</instruction>
